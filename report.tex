...
\subsubsection{On Host A (Server)}
...
sudo venv/bin/python3 vpn.py server \
  ...
  --local-port 5000 \
  --remote-port 5000 \
  --key "<64-byte-key-here>" \
  --tun-local 10.0.0.1 \
  --tun-remote 10.0.0.2
\end{verbatim}
\end{small}

\subsubsection{On Host B (Client)}
...
\hline
1500 bytes & 1556 bytes & 56 bytes & \textbf{3.7\%} \\
\hline
\end{tabular}
\end{center}
...
For 1500-byte packets over 1000 iterations:
...
\textbf{Estimated Grade: 95-100/100}
